...
// ==========================================
const CONFIG = {
  SPREADSHEET_ID: '1tHAlW-Q1HvcwBFvSBfPva1rhv3f3A0n30ScPvF6Wnus',
...
// Main.gs - Add proper header initialization
function initializeSheets() {
    ...
    const sheetHeaders = {
        'Occasions': [
            'ID', 'Date', 'SessionType', 'Status', 'CreatedAt', 'UpdatedAt',
            'TotalRevenue', 'TotalPrizes', 'NetProfit', 'CreatedBy', 'Notes'
        ],
        'SessionGames': [
            'ID', 'OccasionID', 'GameNumber', 'GameName', 'Winners',
            'Prize', 'PrizePerWinner', 'Pattern', 'CreatedAt'
        ],
        'PullTabUsage': [
            'ID', 'OccasionID', 'PullTabID', 'CasesUsed', 'Tickets',
            'Revenue', 'Prizes', 'Profit', 'CreatedAt'
        ],
        'Metrics': [
            'ID', 'OccasionID', 'MetricType', 'MetricName', 'MetricValue',
            'CreatedAt', 'UpdatedAt'
        ],
        'DoorSales': [
            'ID', 'OccasionID', 'ItemCategory', 'ItemName', 'Quantity',
            'UnitPrice', 'TotalAmount', 'CreatedAt'
        ],
        'MoneyCount': [
            'ID', 'OccasionID', 'Denomination', 'Count', 'Value',
            'Type', 'CreatedAt'
        ],
        'FiftyFifty': [
            'ID', 'OccasionID', 'TicketsSold', 'Revenue', 'WinnerPrize',
            'HouseTake', 'CreatedAt'
        ],
        'PullTabLibrary': [
            'ID', 'GameName', 'Manufacturer', 'TicketsPerCase', 'PricePerTicket',
            'TotalRevenue', 'TotalPrizes', 'ProfitPerCase', 'CreatedAt', 'Active'
        ]
    };
    
    // Check and add headers to each sheet
    for (const [sheetName, headers] of Object.entries(sheetHeaders)) {
        try {
            ...
            if (lastColumn === 0 || sheet.getRange(1, 1, 1, headers.length).getValues()[0][0] === '') {
                // Add headers
            ...
            Logger.log(`Error initializing sheet ${sheetName}: ${error}`);
        }
    }
}

// Run initialization on script installation
function onOpen() {
    initializeSheets();
    
    const ui = SpreadsheetApp.getUi();
    ui.createMenu('RLC Bingo Manager')
